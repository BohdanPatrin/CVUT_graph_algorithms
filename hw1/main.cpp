...
// Monster `a` attacks first
    ...
    struct Inventory{
        ...
        std::optional<ItemId> armor;
        std::optional<ItemId> duck;
    ...
    struct ItemHash {
        size_t operator()(const Item& item) const noexcept {
            size_t h1 = std::hash<std::string>()(item.name);
            size_t h2 = std::hash<int>()(item.type);
            size_t h3 = std::hash<int>()(item.hp ^ item.off ^ item.def);
            size_t h4 = std::hash<bool>()(item.stealth);
            return h1 ^ (h2 << 1) ^ (h3 << 2) ^ (h4 << 3);
        }
    };

    Monster heroBase{ 10000, 3, 2 };

    Monster heroWithItems(const Inventory& inv, const std::vector<Item>& all_items) {
        Monster h = heroBase;
        auto apply = [&](std::optional<ItemId> id) {
            if (!id) return;
            const Item& it = all_items[*id];
            h.hp += it.hp;
            h.off += it.off;
            h.def += it.def;
            h.stacking_off += it.stacking_off;
            h.stacking_def += it.stacking_def;
        };
        apply(inv.weapon);
        apply(inv.armor);
        apply(inv.duck);
        if (h.hp <= 0) h.hp = 1;  // hp at minimum 1
        return h;
    }
    ...
    std::vector<Action> find_shortest_path(
        ...
        std::unordered_map<Item, ItemId, ItemHash> itemId;

        // stores the path
        std::unordered_map<State, std::pair<State, Action>, StateHash> parent;

        for (const Room &room: rooms)
            for (const Item &item: room.items)
                if (itemId.find(item) == itemId.end()) {
                    itemId[item] = all_items.size();
                    all_items.push_back(item);
                }

        for (RoomId it: entrances)
            if (!rooms[it].monster || simulate_combat(*rooms[it].monster, heroBase) == B_WINS) {
                State new_st = State{it, Inventory{}, it == treasure, false};

                // check if we've already added this start state
                if (parent.find(new_st) == parent.end()) {
                    q.push_front(new_st);
                    parent[new_st] = {new_st, Move{it}};
                }
            }


        while (!q.empty()) {
            State current_state = std::move(q.front());
            q.pop_front();

            const Inventory& inv = current_state.inv;
            const RoomId room = current_state.room;
            const bool tr = current_state.has_treasure;
            const bool sneaking = current_state.is_sneaking;

            // path reconstruction
            if (tr && std::find(entrances.begin(), entrances.end(), room) != entrances.end()) {
                std::vector<Action> path;
                State s = current_state;
                while (true) {
                    auto& [prev_s, act] = parent.at(s);
                    path.push_back(act);
                    if (s == prev_s) break; // stop when we get the root
                    s = prev_s;
                }
            ...
            if (!sneaking) {
                // pick up every item
                for (size_t local_idx = 0; local_idx < rooms[room].items.size(); ++local_idx) {
                    const Item &it = rooms[room].items[local_idx];
                    Inventory newInv = inv;
                    switch (it.type) {
                        case Item::Weapon:     newInv.weapon.emplace(itemId[it]); break;
                        case Item::Armor:      newInv.armor.emplace(itemId[it]);  break;
                        case Item::RubberDuck: newInv.duck.emplace(itemId[it]);   break;
                        default: break;
                    }

                    State new_st = State{room, newInv, tr, false};
                    if (parent.find(new_st) == parent.end()) {
                        parent[new_st] = {current_state, Pickup{local_idx}};
                        q.push_front(new_st);
                    }
                }

                // drop every item
                auto tryDrop = [&](Item::Type type, const std::optional<ItemId>& slot) {
                    if (slot) {
                        Inventory newInv = inv;
                        switch (type) {
                            case Item::Weapon: newInv.weapon.reset(); break;
                            case Item::Armor: newInv.armor.reset(); break;
                            case Item::RubberDuck: newInv.duck.reset(); break;
                            default: break;
                        }

                        State new_st = {room, newInv, tr, false};
                        if (parent.find(new_st) == parent.end()) {
                            parent[new_st] = {current_state, Drop{type}};
                            q.push_front(new_st);
                        }
                    }
                };
                tryDrop(Item::Weapon, inv.weapon);
                tryDrop(Item::Armor, inv.armor);
                tryDrop(Item::RubberDuck, inv.duck);
            }

            for (RoomId n: rooms[room].neighbors) {
                const Room &nextRoom = rooms[n];
                Monster hero = heroWithItems(inv, all_items);

                bool first_attack =
                        (inv.weapon && all_items[*inv.weapon].first_attack) ||
                        (inv.armor  && all_items[*inv.armor].first_attack)  ||
                        (inv.duck   && all_items[*inv.duck].first_attack);

                bool hasStealth =
                        (inv.weapon && all_items[*inv.weapon].stealth) ||
                        (inv.armor  && all_items[*inv.armor].stealth)  ||
                        (inv.duck   && all_items[*inv.duck].stealth);

                auto canDefeat = [&](const Monster &m) {
                    if (first_attack) return simulate_combat(hero, m) == A_WINS;
                    else return simulate_combat(m, hero) == B_WINS;
                };

                // case 1: room safe or we can win
                if (!nextRoom.monster || canDefeat(*nextRoom.monster)) {
                    State newState = {n, inv, tr || (n == treasure), false};

                    if (parent.find(newState) == parent.end()) {
                        parent[newState] = {current_state, Move{n}};
                        q.push_back(newState);
                    }
                }
                    // case 2: we must sneak
                else if (nextRoom.monster && hasStealth) {
                    State newState = {n, inv, tr, true};
...
const Item stealth_duck{
    ...
    rooms[0].monster = Monster{ .hp = 10'000, .off = 1, .def = 1 };
    rooms[0].neighbors.push_back(1);
    rooms[1].neighbors.push_back(2);
    rooms[1].neighbors.push_back(0);
    rooms[2].neighbors.push_back(1);
    rooms[0].items = {stealth_duck};
    rooms[1].monster = Monster{ .hp = 10'000'000, .off = 1, .def = 1 };
    using student_namespace::find_shortest_path;
    const std::vector<Action> solution = find_shortest_path(rooms, {0}, 2);
