...
std::optional<std::vector<typename SearchProblem::Action>> solve(const SearchProblem& G) {
    ...
    if (G.is_target(start)) {
        return std::vector<Action>{};
    }
    q.push(start);
    visited.insert(start);

    while(!q.empty() && !target_found){
        Vertex v = q.front();
        q.pop();
        for(Action it : G.possible_actions(v)){
            Vertex w = G.next_state(v, it);
    ...
    while (current != start) {
        auto& [prev_state, action_taken] = P.at(current);
        path.push_back(action_taken);
        current = prev_state;
    }
...
struct SuperstitiousHero {
  ...
  SuperstitiousHero(Maze maze, Position hero):_maze(std::move(maze)), _hero(hero) {}

  State initial_state() const { return { _hero, 0 }; }

  std::vector<Action> possible_actions(State v) const { std::vector<Action> ret;
      std::vector<Direction> candidates;

      if (v.moves_mod_5 == 4)
          candidates.push_back(Direction::UP);
      else
          candidates = { Direction::DOWN, Direction::UP, Direction::RIGHT, Direction::LEFT };
  ...
  State next_state(State v, const Action& a) const { Position new_pos = perform_move(v.pos, a);
      int new_moves_mod_5 = (v.moves_mod_5 + 1) % 5;
      return { new_pos, new_moves_mod_5 }; }

  bool is_target(State v) const { return _maze[v.pos.row][v.pos.col] == Tile::EXIT; }
...
struct LimpingKnight {
  struct State {
    Position pos;
    int moves_since_jump;
    friend auto operator <=> (const State&, const State&) = default;
  };
  using Action = LimpingKnightAction;

  LimpingKnight(Maze maze, Position hero) : _maze(std::move(maze)), _hero(hero) {}

  State initial_state() const { return { _hero, 7 }; }

  std::vector<Action> possible_actions(State v) const {
      std::vector<Action> ret;
      ret.push_back(Action::REST);

      for (Action a : { Action::UP, Action::DOWN, Action::LEFT, Action::RIGHT }) {
          Position next_pos = perform_move(v.pos, a);
          if (is_valid_walk(next_pos))
              ret.push_back(a);
      }


      if (v.moves_since_jump >= 7) {
          Action knight_moves[] = {
                  Action::UP_UP_LEFT, Action::UP_UP_RIGHT,
                  Action::DOWN_DOWN_LEFT, Action::DOWN_DOWN_RIGHT,
                  Action::LEFT_LEFT_UP, Action::LEFT_LEFT_DOWN,
                  Action::RIGHT_RIGHT_UP, Action::RIGHT_RIGHT_DOWN
          };
          for (Action a : knight_moves) {
              Position next_pos = perform_move(v.pos, a);
              if (is_valid_walk(next_pos))
                  ret.push_back(a);
          }
  ...
  State next_state(State v, const Action& a) const {
      Position new_pos = perform_move(v.pos, a);

      if (is_knight_move(a))
          return { new_pos, 0 };
      else
          return { new_pos, std::min(v.moves_since_jump + 1, 7) };
      }
  ...
  private:
    ...
    bool is_in_bounds(Position p) const {
        return p.row < _maze.size() && p.col < _maze[p.row].size();
    }

    bool is_valid_walk(Position p) const {
        return is_in_bounds(p) && _maze[p.row][p.col] != Tile::WALL;
    }
};
#endif
...
struct BottleProblem {
  ...
  _target_amount(target_amount), _capacities(bottle_capacities) {}

  State initial_state() const { return State{ std::vector<unsigned>(_capacities.size(), 0) }; }

  std::vector<Action> possible_actions(State v) const {
      std::vector<Action> ret;
      for (size_t i = 0; i < _capacities.size(); ++i) {
          if (v.amounts[i] < _capacities[i])
              ret.push_back(Fill{i});

          if (v.amounts[i] > 0)
              ret.push_back(Empty{i});

          for (size_t j = 0; j < _capacities.size(); ++j) {
              if (i == j) continue;

              if (v.amounts[i] > 0 && v.amounts[j] < _capacities[j])
                  ret.push_back(Transfer{i, j});
          }
      }
      return ret;
  }
  ...
  State next_state(State v, const Action& a) const {
      ...
      std::visit(overloaded{
              [&](const Fill& f) {
                  next_v.amounts[f.index] = _capacities[f.index];
              },
              [&](const Empty& e) {
                  next_v.amounts[e.index] = 0;
              },
              [&](const Transfer& t) {
                  unsigned amount = std::min(next_v.amounts[t.from],
                                             _capacities[t.to] - next_v.amounts[t.to]);
                  next_v.amounts[t.from] -= amount;
                  next_v.amounts[t.to] += amount;
              }
      }, a);
      return next_v;
  }
  ...
  bool is_target(State v) const {
      for (unsigned amt : v.amounts)
          if (amt == _target_amount) return true;
  ...
  private:
    ...
    std::vector<unsigned> _capacities;
};
#endif
